...
export default (ctx, emailParams = {}) => {
  ...
  if (emailParams && emailParams.profile && emailParams.profile) {
    data.profile.name = emailParams.profile.name
    data.profile.avatar = emailParams.profile.avatar
    data.profile.language = emailParams.profile.nativeLanguage || 'en'
  }
  if (emailParams
    && emailParams.request
    && emailParams.request.place
    && emailParams.request.place.name) {
    data.request.place.name = emailParams.request.place.name
  }
  if (emailParams && emailParams.request && emailParams.request.startDate) {
    data.date = moment(emailParams.request.startDate).locale(emailParams._profile.nativeLanguage).format('LL [в] LT').replace('г. ', '')
  }
  return `<!DOCTYPE html>
<html lang="en">
<head>
...
.avatar {float:left;}
.meeting {float:left;line-height:1.5;padding:20px 0px 0px 0px;}
.meeting div b, .jays b {color:#e77e31; font-weight:bold;}
...
.reject {padding:30px 0px 0px 0px;}
.clear {clear:both;}

.content ul.downloads {margin: 20px 0px 30px 0px; padding:0px; height:60px;}
.content ul.downloads li {list-style-type:none;}
.appstore a {background: url('appstore.png') no-repeat;float:right;}
.playstore a {background: url('playstore.png') no-repeat;float:left;}
...
<body>
	<div class="mail">
		...
		<div class="body_mail">
			...
			<div class="content">
				<h1>The meeting is confirmed.</h1>
				<div class="jays">
					<div class="jay">
						<div class="avatar">
              <img src="${ctx.htmlStaticPath}flag.png" class="flag">
              <img src="${data.profile.avatar}" class="jay">
						</div>
						<div class="meeting i">
              <div>The meeting with <b>${data.profile.name}</b></div>
              <div class="place">${data.request.place.name}</div>
              <div class="time">${data.date}</div>
						</div>
						<div class="clear"></div>
					</div>
					<p>Now you can chat with <b>${data.profile.name}</b>.<br>Open his profile to begin chatting.</p>
				</div>
				<ul class="downloads">
				...
				<div class="footer1">
					...
					<p>Write to <a href="mailto:support@hi-jay.eu">support@hi-jay.eu</a>, if you have any problems when using "Hi, Jay!", we will surely help you.
					<p>Best wishes, "Hi, Jay!" team.
					<p><a href="http://www.hi-jay.eu" target="_blank">www.hi-jay.eu</a>
					<p class="unsibscribe">To unsubscribe from these mails, pass into the application in the section "My Profile".
				</div>
			</div>
		</div>
		<div class="lines">
