...
export default (ctx, emailParams) => {
  ...
  if (emailParams && emailParams.profile) {
    data.profile.name = emailParams.profile.name
    data.profile.avatar = emailParams.profile.avatar
    data.profile.language = emailParams.profile.nativeLanguage
  }
  if (emailParams && emailParams.request) {
    if (emailParams.request.startDate) {
      data.request.startDate = data.date = moment(emailParams.request.startDate)
      .locale(emailParams._profile.nativeLanguage)
      .format('D MMMM Y [at] LT')
    }
    if (emailParams.request.place && emailParams.request.place.name) {
      data.request.place.name = emailParams.request.place.name
    }
    if (emailParams.request.place && emailParams.request.place.address) {
      data.request.place.address = `(${emailParams.request.place.address})`
    }
  }
  return `<!DOCTYPE html>
<html lang="en">
...
<body>
	<div class="mail">
		...
		<div class="body_mail">
			...
			<div class="content">
				<h1>You are invited on the meeting!<br>Jay is waiting for your answer.</h1>
				<div class="jays">
					<div class="jay">
						<div class="avatar">
						...
						<div class="meeting i">
								<div><b>${data.profile.name}</b> suggests you help in language learning.</div>
					...
					<div>
						<h2>To answer the suggestion:</h2>
						<ol>
							<li>Open "Hi, Jay!" app
							<li>Go to "'My meetings" section, there look for "Requests" subsection
							<li>Go to the necessary Jay's profile
						</ol>
					</div>
				</div>
				<ul class="downloads">
